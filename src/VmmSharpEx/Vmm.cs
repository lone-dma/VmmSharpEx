...
public sealed partial class Vmm : IDisposable
{
    ...
    public unsafe LeechCore.LcScatterHandle MemReadScatter(uint pid, VmmFlags flags, params Span<ulong> vas)
    {
        ...
        _ = Vmmi.VMMDLL_MemReadScatter(_handle, pid, pppMEMs, (uint)vas.Length, (uint)flags);
    ...
    public unsafe byte[]? MemRead(uint pid, ulong va, uint cb, out uint cbRead, VmmFlags flags = VmmFlags.NONE)
    {
        ...
        fixed (byte* pb = arr)
        {
            if (!Vmmi.VMMDLL_MemReadEx(_handle, pid, va, pb, cb, out cbRead, (uint)flags))
            {
                return null;
            }
        }
    ...
    {
        return Vmmi.VMMDLL_MemReadEx(_handle, pid, va, (byte*)pb, cb, out cbRead, (uint)flags);
    }
        ...
        where T : unmanaged, allows ref struct
    {
        ...
        {
            return Vmmi.VMMDLL_MemReadEx(_handle, pid, va, (byte*)pb, cb, out var cbRead, (uint)flags) && cbRead == cb;
        }
    }
        ...
        where T : unmanaged
    {
        ...
        fixed (T* pb = array)
        {
            if (!Vmmi.VMMDLL_MemReadEx(_handle, pid, va, (byte*)pb, cb, out var cbRead, (uint)flags) || cbRead != cb)
            {
                return null;
            }
        }
    ...
    public unsafe ThreadCallstackEntry[]? Map_GetThread_Callstack(uint pid, uint tid, VmmFlags flags = VmmFlags.NONE)
    {
        ...
        try
        {
            if (!Vmmi.VMMDLL_Map_GetThread_Callstack(_handle, pid, tid, (uint)flags, out pMap))
            {
                return null;
            }
    ...
    public unsafe byte[]? WinReg_HiveReadEx(ulong vaCMHIVE, uint ra, uint cb, VmmFlags flags = VmmFlags.NONE)
    {
        ...
        fixed (byte* pb = data)
        {
            if (!Vmmi.VMMDLL_WinReg_HiveReadEx(_handle, vaCMHIVE, ra, pb, cb, out cbRead, (uint)flags))
            {
                return null;
            }
        }
