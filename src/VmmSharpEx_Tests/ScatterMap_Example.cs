...
namespace VmmSharpEx_Tests
{
    internal static class ScatterMap_Example
    {
        public static void Example()
        {
            ...
            {
                int i = ix; // Capture
                r1[i].AddEntry<ulong>(0, list[i] + 0x10); // Read a pointer type
                r1[i].AddEntry<byte[]>(1, list[i] + 0x100, 64); // Read a byte array of 64 bytes
                r1[i].Completed += (sender, cb1) =>
                {
                    if (cb1.TryGetResult<ulong>(0, out var ptr))
                    {
                        // Do stuff with this pointer if read is successful!
                        Console.WriteLine($"Pointer: 0x{ptr:X}");
                        r2[i].AddEntry<uint>(0, ptr + 0x20); // Use this pointer in the next round to read a uint
                        r2[i].Completed += (sender, cb2) =>
                        {
                            if (cb2.TryGetResult<uint>(0, out var val))
                            {
                    ...
                    if (cb1.TryGetResult<byte[]>(1, out var bytes))
                    {
