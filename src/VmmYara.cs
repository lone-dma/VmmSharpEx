...
public sealed class VmmYara : IDisposable
{
    ...
    public VmmYara(Vmm hVmm, uint pid, string[] yara_rules, ulong addr_min = 0, ulong addr_max = ulong.MaxValue, uint cMaxResult = 0, uint readFlags = 0)
    {
    ...
    private void Start_DoWork()
    {
        ...
        var ptrTerms = new IntPtr[arrTerms.Length];
        var gchTerms = new GCHandle[arrTerms.Length];
        for (var i = 0; i < arrTerms.Length; i++)
        {
            var utf8Bytes = Encoding.UTF8.GetBytes(arrTerms[i] + '\0'); // Add null terminator
            gchTerms[i] = GCHandle.Alloc(utf8Bytes, GCHandleType.Pinned);
            ptrTerms[i] = gchTerms[i].AddrOfPinnedObject();
        }

        var gchTermsArray = GCHandle.Alloc(ptrTerms, GCHandleType.Pinned);
        var ptrTermsArray = gchTermsArray.AddrOfPinnedObject();
        _native.cRules = (uint)_terms.Count;
        _native.pszRules = ptrTermsArray;
        // 2: call native:
        Marshal.StructureToPtr(_native, _ptrNative, false);
        var fResult = Vmmi.VMMDLL_YaraSearch2(_hVmm, _PID, _ptrNative, IntPtr.Zero, IntPtr.Zero);
        // 3: finish / clean up:
        gchTermsArray.Free();
        foreach (var gch in gchTerms) gch.Free();
    ...
    private unsafe bool YaraResultCallback(IntPtr ctx, Vmmi.VMMYARA_RULE_MATCH pRuleMatch, byte* pbBuffer, ulong cbBuffer)
    {
        ...
        var addrBase = (ulong)Marshal.ReadInt64(ctx, Marshal.OffsetOf<Vmmi.VMMDLL_YARA_CONFIG>("vaCurrent").ToInt32());
        match.sRuleIdentifier = pRuleMatch.szRuleIdentifier;
        // tags:
        match.tags = new string[pRuleMatch.cTags];
        for (var i = 0; i < pRuleMatch.cTags; i++) match.tags[i] = Marshal.PtrToStringUTF8(pRuleMatch.szTags[i]);
        // meta:
        match.meta = new string[pRuleMatch.cMeta][];
        for (var i = 0; i < pRuleMatch.cMeta; i++) match.meta[i] = new string[2] { pRuleMatch.Meta[i].szIdentifier, pRuleMatch.Meta[i].szString };
        // strings:
        match.strings = new YaraMatchString[pRuleMatch.cStrings];
        for (var i = 0; i < pRuleMatch.cStrings; i++)
        {
            match.strings[i].sString = pRuleMatch.Strings[i].szString;
            match.strings[i].addresses = new ulong[pRuleMatch.Strings[i].cMatch];
            for (var j = 0; j < pRuleMatch.Strings[i].cMatch; j++) match.strings[i].addresses[j] = addrBase + pRuleMatch.Strings[i].cbMatchOffset[j];
        }
